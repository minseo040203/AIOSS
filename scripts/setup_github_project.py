#!/usr/bin/env python3
"""
GitHub Project 및 스프린트 설정 스크립트
칸반 기반 프로젝트 보드, 라벨, 마일스톤, 이슈를 자동으로 생성합니다.
"""
...
class GitHubProjectSetup:
    def __init__(self):
        self.owner = "minseo040203"
        self.repo = "AIOSS"
        self.project_title = "AIOSS Development Sprint"
    ...
    def create_milestones(self):
        ...
        for ms in milestones:
            cmd = (
                f'gh milestone create "{ms["title"]}" '
                f'--description "{ms["description"]}" '
                f'--repo {self.owner}/{self.repo}'
            )
            self.run_command(cmd)
            print(f"  ✓ '{ms['title']}' 마일스톤 생성 완료")
    ...
    def create_issues(self):
        ...
        for i, issue in enumerate(issues, 1):
            # 마일스톤 찾기 (생성하면서 ID를 알아야 함)
            milestone_arg = ""
            if issue.get("milestone"):
                milestone_arg = f'--milestone "{issue["milestone"]}"'
            
            # 라벨 옵션
            labels_arg = " ".join([f'--label "{label}"' for label in issue.get("labels", [])])
            ...
            if output:
                print(f"  ✓ Issue #{i}: '{issue['title']}' 생성 완료")
                issue_urls.append(output)
            else:
                print(f"  ⚠ Issue #{i} 생성 실패: {issue['title']}")
        
        print(f"✅ {len(issue_urls)}/{len(issues)}개 이슈 생성 완료!")
        return issue_urls
    ...
    def create_project(self):
        """GitHub Project (Kanban) 생성"""
        print("\n🎨 GitHub Project 생성 중...")
        
        # Note: GitHub CLI로 Project 생성은 아직 완전히 지원되지 않음
        # 대신 Python API를 사용하거나 고급 GUI로 진행 안내
        print("""
⚠️  GitHub Project 칸반 보드 수동 생성 필요:

1. GitHub 저장소로 이동: https://github.com/minseo040203/AIOSS
2. 'Projects' 탭 클릭
3. '새 프로젝트' 버튼 클릭
4. 프로젝트 이름: 'AIOSS Development Sprint'
5. 템플릿: '칸반' 선택
6. 생성 후 컬럼 편집:
   - Backlog (백로그)
   - To Do (할 일)
...
또는 다음 GitHub CLI 명령을 실행하세요 (Node.js 필요):
  npx github-cli-projects create --owner minseo040203 --repo AIOSS
        """)
        
        print("✅ Project 설정 가이드를 제공했습니다!")
    ...
    def run_all(self):
        ...
        try:
            ...
            print("=" * 70)
            print("\n📊 다음 단계:")
            print("1. GitHub 프로젝트 보드 수동 생성 또는 설정")
            print("2. 생성된 이슈들을 프로젝트 칸반 보드에 추가")
            print("3. 팀원들과 함께 스프린트 계획 회의 진행")
